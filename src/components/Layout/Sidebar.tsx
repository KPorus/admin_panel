...
const SideBar = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className='relative flex'>
      <div
        className={`flex transition-all bg-[#001529] ${
          toggle ? "w-[8%]" : "w-[8%] md:w-[15%]"
        } min-h-[100vh] justify-center`}>
        <div className='flex flex-col justify-between items-center'>
          <div className='fixed flex justify-center items-center flex-col py-5'>
            <Link
              href={"/"}
              className={`link link-body py-[0.6rem] text-center ${
                toggle ? "w-[3rem]" : "w-[3rem] md:w-[9rem]"
          ...
          <button
            className={`fixed bottom-0 text-center ${
              toggle ? "w-[3rem]" : "w-[3rem] md:w-[9rem]"
